...
class Notebook(object):
    ...
    def create_new_worksheet(self, worksheet_name, username, add_to_list=True,
        ...
        r"""
        Creates a new worksheet with the given parameters.
        
        INPUT:
        
        - ``worksheet_name`` - a string

        - ``username`` - a string

        EXAMPLES:
        We create a notebook and make a worksheet in it.
        
        ::

            sage: nb = sagenb.notebook.notebook.Notebook(tmp_dir()+'.sagenb')
            sage: nb.create_new_worksheet('name','admin',id='id',subpath='subpath')
            admin/subpath/id: [Cell 1: in=, out=]
        ...
        if(id != None):
            if subpath:
                if "%s/%s/%s"%(username, subpath, id) in self.__worksheets:
                    raise ValueError("worksheet with given subpath and id already exists")
            elif "%s/%s"%(username,id) in self.__worksheets:
                raise ValueError("worksheet with given subpath and id already exists")
    ...
    def new_worksheet_with_title_from_text(self, text, owner, id=None, subpath=None):
        name, _ = worksheet.extract_name(text)
        W = self.create_new_worksheet(name, owner, id=id, subpath=subpath)
        return W
    ...
    def import_worksheet(self, filename, owner, id=None, subpath=None):
        r"""
        ...
        if ext.lower() == '.txt':
            ...
            W = self._import_worksheet_txt(filename, owner, id=id, subpath=subpath)
        elif ext.lower() == '.sws':
            # An sws file (really a tar.bz2) which defines a worksheet with graphics, etc.
            W = self._import_worksheet_sws(filename, owner, id=id, subpath=subpath)
        elif ext.lower() == '.html':
            ...
            if re.search(cell_pattern, html, re.DOTALL) is not None:
                ...
                W = self._import_worksheet_docutils_html(filename, owner, id=id, subpath=subpath)
            elif re.search(sphinx_pattern, html) is not None:
                W = self._import_worksheet_html(filename, owner, id=id, subpath=subpath)
            else:
            ...
            W = self._import_worksheet_rst(filename, owner, id=id, subpath=subpath)
        else:
    ...
    def _import_worksheet_txt(self, filename, owner, id=None, subpath=None):
        r"""
        ...
        using this function.::
            ...
            sage: name = tmp_filename() + '.txt'
            sage: open(name,'w').write('foo\n{{{\na = 10\n}}}')
            sage: W = nb._import_worksheet_txt(name, 'admin', id='worksheet', subpath='subpath'); W
            admin/subpath/worksheet: [TextCell 0: foo, Cell 1: in=a = 10, out=]
        """
        ...
        worksheet = self.new_worksheet_with_title_from_text(worksheet_txt, owner, id=id, subpath=subpath)
        # Set the new worksheet to have the contents specified by that file.
        worksheet.edit_save(worksheet_txt)
        return worksheet

    def _import_worksheet_sws(self, filename, username, id=None, subpath=None):
        r"""
        ...
        file first.::
            ...
            sage: W = nb.import_worksheet(name, 'admin', id='ws1',subpath='subpath')
            sage: W.filename()
            'admin/subpath/ws1'
        ...
        Now we import the sws.::
            ...
            sage: W = nb._import_worksheet_sws(sws, 'admin', id='ws2', subpath='subpath')
            sage: nb._Notebook__worksheets[W.filename()] = W

        Yes, it's there now (as a new worksheet)::

            sage: sorted([w.filename() for w in nb.get_all_worksheets()])
            ['admin/subpath/ws1', 'admin/subpath/ws2']
        """
        if id == None:
            id = self.new_id_number(username)
        worksheet = self.__storage.import_worksheet(username, id, filename, subpath=subpath)
    ...
    def _import_worksheet_sws_with_old_id(self, filename, username, subpath=None):
        r"""
        Import an sws format worksheet into this notebook as a new
        worksheet, preserving the last part of the imported worksheet's 
        filename.
    ...
    def _import_worksheet_html(self, filename, owner, id=None, subpath=None):
        r"""
    ...
    def _import_worksheet_rst(self, filename, owner, id=None, subpath=None):
        r"""
        ...
        # Create worksheet
        worksheet = self.create_new_worksheet(title, owner, id=id, subpath=subpath)
        worksheet.edit_save(worksheet_txt)

        return worksheet

    def _import_worksheet_docutils_html(self, filename, owner, id=None, subpath=None):
        r"""
